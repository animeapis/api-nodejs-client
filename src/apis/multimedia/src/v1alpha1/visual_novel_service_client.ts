...
export class VisualNovelServiceClient {
   ...
   *     const client = new VisualNovelServiceClient({fallback: 'rest'}, gax);
   *     ```
   */
 ...
 */
  async checkReconcileVisualNovelsProgress(name: string): Promise<LROperation<protos.animeshon.multimedia.v1alpha1.ReconcileVisualNovelsResponse, protos.animeshon.multimedia.v1alpha1.OperationMetadata>>{
    ...
    const decodeOperation = new this._gaxModule.Operation(operation, this.descriptors.longrunning.reconcileVisualNovels, this._gaxModule.createDefaultBackoffSettings());
 ...
 */
   ...
   */
  close(): Promise<void> {
    if (this.visualNovelServiceStub && !this._terminated) {
